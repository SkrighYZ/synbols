...
def color_sampler(rng=np.random, brightness_range=(0, 1), zero_dim=None):
    def sampler():
        b_delta = brightness_range[1] - brightness_range[0]
        c = rng.rand(3) * b_delta + brightness_range[0]
        if not zero_dim is None:
            c[zero_dim] /= 3
        return c
...
class Gradient(RandomPattern):
    ...
    def __init__(self, alpha=1, types=("radial", "linear"), random_color=None, seed=None, zero_dim=None):
        self.random_color = random_color
        self.seed = seed
        self.rng = np.random.RandomState(self.seed)
        self.zero_dim = zero_dim

        self.types = types
        self.alpha = alpha

    def set_as_source(self, ctxt):
        pat = _random_pattern(self.alpha, self.random_color, rng=self.rng, patern_types=self.types, zero_dim=self.zero_dim)
        ctxt.set_source(pat)
...
def _random_pattern(alpha=0.8, random_color=None, patern_types=("linear", "radial"), rng=np.random, zero_dim=None):
    """"Select a random pattern with either radial or linear gradient."""
    if random_color is None:
        random_color = color_sampler(rng, zero_dim=zero_dim)
...
class ImagePattern(RandomPattern):
    ...
    def __init__(self, root="/images", rotation=0, translation=0.0, crop=True, min_crop_size=0.2, seed=None, aug_seed=None):
        ...
        self.rng = np.random.RandomState(self.seed)
        self.aug_rng = np.random.RandomState(self.aug_seed)

    def _rotate_and_translate(self, im, rotation, translation):
        """Randomly rotate and translate the image."""
        w, h = im.size
        rot = np.rad2deg(rotation) * ((self.aug_rng.rand() - 0.5) * 2)
        translation_x = w * translation * ((self.aug_rng.rand() - 0.5) * 2)
        translation_y = h * translation * ((self.aug_rng.rand() - 0.5) * 2)
    ...
    def _random_crop(self, im, min_crop_size):
        ...
        crop_x2 = self.aug_rng.randint(min_crop_size_x + 1, w)
        crop_y2 = self.aug_rng.randint(min_crop_size_y + 1, h)
        x1 = self.aug_rng.randint(0, crop_x2 - min_crop_size_x)
        y1 = self.aug_rng.randint(0, crop_y2 - min_crop_size_y)
